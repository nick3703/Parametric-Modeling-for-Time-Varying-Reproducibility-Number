...
Projections<-OC.proj.IT.obs%>%filter(dates>lubridate::today())
Projection.df<-data.frame(new.cases=diff(Projections$I),dates=seq(from = lubridate::today()+1, to = lubridate::today()+nrow(Projections)-1, by = 'day'))
Projections2<-OC.proj.CH.obs%>%filter(dates>lubridate::today())
Projection2.df<-data.frame(new.cases=diff(Projections2$I),dates=seq(from = lubridate::today()+1, to = lubridate::today()+nrow(Projections2)-1, by = 'day'))
...
########What if
N<-Population
OC.proj.base<-data.frame(time=0,S=N-Exposed.init-Infected.init,E=Exposed.init,I=Infected.init)
for (j in 1:length(R0.Wuh)){
  #SEIR Model
  #Parameter Values
  parameters_values <- c(
    N = N, #Total Population
    R0 = as.numeric(2.5), #R0 value
    DI = 8, #Infectious Period
    DE = 5 #Incubation Period
  )
  seir_equations <- function(time, variables, parameters) {
  ...
  Exposed<-OC.proj.base[j,]$E
  Infected<-OC.proj.base[j,]$I
  initial_values <- c(
    S = N-Exposed-Infected, # number of susceptibles at time = 0
    E = Exposed, # number of exposed at time = 0
    I = Infected # number of infectious at time = 0
  ...
  OC.proj.base[(j+1),]<-sir_values_1[2,]
  N<-as.numeric(sir_values_1[2,2]+sir_values_1[2,3]+sir_values_1[2,4])
...
#########
#Add in the number of recovered, allowing for the use of the total case count
Recovered <- Population-(rowSums(OC.proj.base[,2:4])) # Calculate the number of recovered, using the fact that N = S + E + I + R
#OC.proj.CH$I <- OC.proj.CH$I + OC.proj.CH$E + Recovered
OC.proj.base$I <- OC.proj.base$I + Recovered
#########

#projects the Estimated Actual Cases [what the hospitals would report if ALL infections were confirmed] 
#in Orange County, NY if OC follows the estimated R(t) of Hubei Province 
OC.proj.base<-OC.proj.base %>% mutate(legend="Estimated Actual Cases",dates=seq(from = lubridate::today()-k, to = lubridate::today()+j-k,                                                                          by = 'day'))%>% dplyr::select(-S,-E)

#projects the Estimated Reported Cases [what the hospitals will likely report as not ALL infections will be confirmed] 
#in Orange County, NY if OC follows the estimated R(t) of Hubei Province 
OC.proj.base.obs<-data.frame(time=OC.proj.base$time,I=OC.proj.base$I/4,legend="Estimated Reported Cases",dates=seq(from = lubridate::today()-k, to = lubridate::today()+j-k, by = 'day'))

#group the results
OC.base.tot<-rbind(OC.proj.base,OC.proj.base.obs)


colors <- c("Projected Actual Cases (Italy)" = "green4", "Projected Reported Cases (Italy)" = "green4", 
            "Projected Actual Cases (Hubei)" = "red2", "Projected Reported Cases (Hubei)" = "red2", 
            "Observed Reported Cases" = "black",
            "Projected Actual Cases (baseline)" = "blue", 
            "Projected Reported Cases (baseline)" = "blue")
...
ggplot() +
  ...
  geom_line(aes(x=dates,y=I,color="Projected Actual Cases (Hubei)"), OC.CH.tot[1:nrow(OC.proj.IT),], lwd=1, lty=2) +
  geom_line(aes(x=dates,y=I,color="Projected Reported Cases (Hubei)"), OC.CH.tot[(nrow(OC.CH.tot)/2+1):(nrow(OC.CH.tot)/2+nrow(OC.proj.IT)),], lwd=1) +
  geom_line(aes(x=dates,y=I,color="Projected Actual Cases (baseline)"), OC.base.tot[1:nrow(OC.proj.IT),], lwd=1, lty=2) +
  geom_line(aes(x=dates,y=I,color="Projected Reported Cases (baseline)"), OC.base.tot[(nrow(OC.base.tot)/2+1):(nrow(OC.base.tot)/2+nrow(OC.proj.IT)),], lwd=1) +
  geom_point(aes(x=dates,y=I,color="Observed Reported Cases"), OC.df,size=2.5) +
  labs(y="Number of Infected Personnel",x="Date",color="Legend") +
  theme(legend.position = "bottom") +
  scale_color_manual(values = colors) +
  theme(legend.text=element_text(size=8)) +
  guides(colour = guide_legend(override.aes = list(shape = c(16,NA,NA,NA,NA,NA,NA),
